...
public class course {
    ...
    public void insertUpdateDeleteStudent(char operation,Integer id,String label,Integer hours)
    {
       Connection con= MyConnection.getConnection();
           ...
           if(operation=='d') //u for update
           {
               int YesOrNo = JOptionPane.showConfirmDialog(null, "The Scores will be also deleted","Delete Course",JOptionPane.OK_CANCEL_OPTION,0);
   ...
   public boolean isCourseExist(String courseName,int hours)
   {
       boolean isExist = false;
       Connection con = MyConnection.getConnection();
        PreparedStatement ps;
        try {
            ps = con.prepareStatement("SELECT * FROM `course` WHERE `label` = ? AND `hours_number` = ?");
   ...
   public int getCourseId(String courseLabel1){
       int courseId = 0;
       Connection con = MyConnection.getConnection();
        PreparedStatement ps;
        try {
